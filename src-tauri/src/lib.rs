...
struct Paper {
  ...
  created_at: String, updated_at: String, deleted_at: Option<String>
}
#[derive(Clone, Serialize, Deserialize)] #[serde(rename_all="camelCase")]
...
async fn load_snapshot(pool: &SqlitePool, dir: &Path) -> Result<LibrarySnapshot> {
  ...
  Ok(LibrarySnapshot { papers,categories,tags,annotations,vocabulary,figures,excerpts,tasks,views,profile,llm,metadata,library_path:dir.to_string_lossy().into_owned() })
}
...
async fn import_pdfs(state:State<'_,AppState>,paths:Vec<String>)->Result<Vec<ImportedPaper>> {
  ...
  for source in paths {
    ...
    let id=Uuid::new_v4().to_string(); let rel=format!("pdf/originals/{id}.pdf"); fs::copy(&source,state.library_dir.join(&rel)).map_err(err)?;
    let title=source.file_stem().and_then(|v|v.to_str()).unwrap_or("Untitled paper").replace('_'," "); let now=Utc::now().to_rfc3339();
    let paper=Paper{id,title_en:title,title_zh:None,authors:vec![],category_id:None,tag_ids:vec![],status:"unread".into(),summary:None,abstract_en:None,abstract_zh:None,venue:None,publication_date:None,doi:None,arxiv_id:None,source_url:None,pdf_path:Some(rel),pdf_sha256:None,page_count:None,has_text_layer:None,favorite:false,reading_page:Some(1),created_at:now.clone(),updated_at:now,deleted_at:None};
...
pub fn run() {
  tauri::Builder::default().plugin(tauri_plugin_dialog::init()).setup(|app| { let (dir,location_config)=resolve_library_dir(app).map_err(std::io::Error::other)?;let pool=tauri::async_runtime::block_on(open_pool(&dir)).map_err(std::io::Error::other)?;app.manage(AppState{library_dir:dir,location_config,pool:RwLock::new(pool)});Ok(()) })
    .invoke_handler(tauri::generate_handler![initialize_library,save_paper,save_annotation,delete_annotation,save_vocabulary,save_excerpt,save_task,delete_task,save_figure,save_category,save_tag,merge_taxonomy,save_view,save_profile,save_llm_settings,save_online_metadata_settings,lookup_online_metadata,test_llm_connection,translate_text,analyze_paper_with_llm,find_duplicate_candidates,import_pdfs,import_citation_files,read_managed_file,write_export_file,index_pdf_pages,indexed_pdf_pages,ocr_page_image,prepare_library_relocation,search_library,create_backup,restore_backup])
