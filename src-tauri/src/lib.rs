...
async fn initialize_pool(pool: &SqlitePool, dir: &Path) -> Result<()> {
  ...
  sqlx::query("CREATE TABLE IF NOT EXISTS paper_explain_chats (
    id TEXT PRIMARY KEY,
  ...
  sqlx::query("CREATE INDEX IF NOT EXISTS idx_paper_explain_chats_paper ON paper_explain_chats(paper_id, updated_at)").execute(pool).await.map_err(err)?;
  let has_active_chat: bool = sqlx::query("PRAGMA table_info(paper_explains)").fetch_all(pool).await.map_err(err)?
    .iter().any(|row| row.get::<String, _>("name") == "active_chat_id");
  if !has_active_chat {
    sqlx::query("ALTER TABLE paper_explains ADD COLUMN active_chat_id TEXT").execute(pool).await.map_err(err)?;
  }
  crate::explain::migrate_legacy_explain_chats(pool).await?;
  sqlx::query("INSERT INTO settings(key,value) VALUES('schema_version','6') ON CONFLICT(key) DO UPDATE SET value='6'").execute(pool).await.map_err(err)?;
...
#[tauri::command] async fn indexed_pdf_pages(state:State<'_,AppState>,paper_id:String)->Result<Vec<PageText>> { let pool=state.pool.read().await; sqlx::query("SELECT page,text FROM pdf_pages WHERE paper_id=? ORDER BY page").bind(paper_id).fetch_all(&*pool).await.map_err(err).map(|rows|rows.into_iter().map(|row|PageText{page:row.get("page"),text:row.get("text")}).collect()) }
