  ...
  return <main className="content-page tasks-page">
    ...
    <div className="task-layout">
      <section className="task-list-panel"><header><div><h2>任务清单</h2><small>按截止日期排序</small></div><button className="ghost" onClick={() => { setEditing(undefined); setNewDate(today); }}><Plus size={16} /></button></header>
        <div className="task-list">{tasks.length === 0 ? <div className="task-empty">还没有任务。可从阅读计划开始。</div> : tasks.map(task => <article className={"task-row " + (task.status === "done" ? "done" : "")} key={task.id}>
          <button className="task-check" aria-label={task.status === "done" ? "恢复任务" : "完成任务"} onClick={() => void toggle(task)}>{task.status === "done" ? "✓" : ""}</button>
          <button className="task-main" onClick={() => setEditing(task)}><strong>{task.title}</strong><span>{dateLabel(task.dueDate)} · {statusLabel[task.status]}{paperTitle(task.paperId) ? " · " + paperTitle(task.paperId) : ""}</span></button>
