...
export function LlmSettingsForm() {
  ...
  return <section className="settings-form llm-settings"><h2><Sparkles size={19} />LLM 自动整理</h2><p className="settings-description">支持 OpenAI 兼容的 Chat Completions 接口。导入新 PDF 时，应用先做本地文本提取，再将论文内容和最多 3 张候选框架图页发送到你配置的模型。阅读台收录术语/写作句时，有 API Key 则优先用 LLM 做学术翻译。</p>
    ...
    <label className="checkbox-setting"><input type="checkbox" checked={value.autoAnalyzeOnImport} onChange={e => setValue(v => v && { ...v, autoAnalyzeOnImport: e.target.checked })} />导入 PDF / 雷达入库后自动用 LLM 整理（雷达路径仅填空，不覆盖已有中文摘要）</label>
    <label className="checkbox-setting"><input type="checkbox" checked={value.autoClassifyOnImport} onChange={e => setValue(v => v && { ...v, autoClassifyOnImport: e.target.checked })} />导入 / 雷达入库时自动分类（主领域与子领域，仅使用设置页现有词表）</label>
    ...
    <p className="settings-description">
      ...
      {" "}是科学文本语言模型（MLM），不能当作 OpenAI Embeddings API 的模型名，也不适合直接做兴趣短语语义匹配。
      本地改用学术友好的句子向量模型{" "}
    ...
    <div className="info-card">
      <HardDrive size={18} />
      <div>
        <strong>{usingLocal ? "当前：本地向量已启用" : localEmbed?.installed ? "模型文件已在本地，尚未写入 Embeddings 字段" : "尚未安装本地向量模型"}</strong>
        <p>
          {localEmbed
