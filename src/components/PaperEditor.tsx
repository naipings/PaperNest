...
export function PaperEditor({ initial, categories, tags, onSave, onCancel }: { initial?: Paper; categories: Category[]; tags: Tag[]; onSave(paper: Paper): Promise<void>; onCancel(): void }) {
  ...
  return <form className="paper-editor" onSubmit={submit}>
    ...
    <fieldset><legend>标签</legend><div className="tag-checks">{tags.map(tag => <label key={tag.id}><input type="checkbox" checked={paper.tagIds.includes(tag.id)} onChange={e => set("tagIds", e.target.checked ? [...paper.tagIds, tag.id] : paper.tagIds.filter(id => id !== tag.id))} /><span style={{ borderColor: tag.color }}>{tag.name}</span></label>)}</div></fieldset>
