...
import { Archive, Database, Download, HardDrive, Laptop, LockKeyhole, Palette, Radar, RotateCcw, Save, Search, Settings, ShieldCheck, SlidersHorizontal, Sparkles, Tags as TagsIcon, UserRound } from "lucide-react";
...
import { RadarSettingsForm } from "./RadarSettingsForm";
import { ResearchSettingsForm } from "./ResearchSettingsForm";
export function SettingsView() {
  ...
  if (!data) return null; const sections = [{ id: "profile", label: "个人资料", icon: UserRound }, { id: "llm", label: "LLM 自动整理", icon: Sparkles }, { id: "research", label: "文献调研", icon: Search }, { id: "metadata", label: "在线元数据", icon: Database }, { id: "radar", label: "论文雷达", icon: Radar }, { id: "customFields", label: "自定义字段", icon: SlidersHorizontal }, { id: "taxonomy", label: "分类与标签", icon: TagsIcon }, { id: "data", label: "本地数据", icon: HardDrive }, { id: "privacy", label: "隐私与安全", icon: ShieldCheck }];
  return <main className="settings-page"><header className="page-heading">
    ...
    {section === "research" && <ResearchSettingsForm />}
    {section === "metadata" && <OnlineMetadataSettingsForm />}
    {section === "radar" && <RadarSettingsForm />}
...
function ProfileForm({ profile, onSave }: { profile: Profile; onSave(value: Profile): Promise<void> }) {
  ...
  return <form className="settings-form" onSubmit={e => { e.preventDefault(); void onSave(value); }}>
    ...
    <p className="settings-description">保存在本机，用于界面展示个人偏好。</p>
