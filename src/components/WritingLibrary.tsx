...
export function WritingLibrary({ onOpenPaper }: { onOpenPaper(paper: Paper, page?: number): void }) {
  ...
  const remove = async (ids: string[]) => {
    ...
    try {
      ...
      setNotice(`删除写作素材失败：${error instanceof Error ? error.message : String(error)}`);
    }
  };
  const updatePurpose = async (item: typeof excerpts[number], next: string) => {
    if (next === item.purpose) return;
    try {
      await saveExcerpt({ ...item, purpose: next });
    } catch (error) {
      setNotice(`更新写作用途失败：${error instanceof Error ? error.message : String(error)}`);
    }
  };
  const copySource = async (sourceText: string) => {
    try {
      await navigator.clipboard.writeText(sourceText);
    } catch (error) {
      setNotice(`复制原文失败：${error instanceof Error ? error.message : String(error)}`);
    }
  };
  return <main className="content-page"><header className="page-heading">
    ...
    <div className="excerpt-grid">{excerpts.map(item => {
      ...
      return <article className={`excerpt-card ${checkedIds.has(item.id) ? "checked" : ""}`} key={item.id}><header><div className="excerpt-card-meta"><label className="excerpt-check"><input className="row-check" type="checkbox" checked={checkedIds.has(item.id)} onChange={() => toggle(item.id)} aria-label="选择写作素材" /></label><div className="excerpt-purpose"><FilterMenu value={item.purpose} onChange={next => void updatePurpose(item, next)} groups={[{ title: "写作用途", options: labels.map(label => ({ id: label, label })) }]} /></div></div><div className="excerpt-card-actions"><button type="button" title="复制" onClick={() => void copySource(item.sourceText)}><Copy size={15} /></button><button type="button" className="annotation-delete" title="删除" onClick={() => void remove([item.id])}><Trash2 size={15} /></button></div></header><blockquote>“{item.sourceText}”</blockquote><p>{item.translationZh || "中文翻译待补充"}</p>{item.personalRewrite && <div className="rewrite"><small>我的改写</small>{item.personalRewrite}</div>}<footer><button disabled={!paper} onClick={() => paper && onOpenPaper(paper, item.page)}><BookOpen size={14} />{paper?.titleZh || paper?.titleEn || "来源论文"} · P{item.page ?? "?"}</button></footer></article>;
    })}</div>
  </main>;
}
