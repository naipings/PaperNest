...
export function DetailPanel({ paper, onClose, onOpenPdf, onSelect }: { paper: Paper; onClose(): void; onOpenPdf(paper: Paper): void; onSelect(paper: Paper): void }) {
  const { data, savePaper, saveVocabulary, deleteVocabulary, saveFigure } = useLibrary(); const [tab, setTab] = useState<Tab>("overview"); const [editing, setEditing] = useState(false); const [newTerm, setNewTerm] = useState(false); const [checkedTermIds, setCheckedTermIds] = useState<Set<string>>(new Set());
  if (!data) return null;
