...
4. 阅读台用 PDF.js 连续滚动阅读：普通滚轮上下翻页，**Ctrl + 滚轮**缩放，顶栏提供高亮、下划线、便签批注、手绘等工具。
...
1. 高亮、下划线：切到对应工具后拖选文字。
2. **批注便签**：切到批注模式后单击页面放置，或拖选文字后标注；便签可拖动、悬停预览、双击编辑，右键复制或删除。
3. 拖选英文词语或句子，使用浮动工具栏「收为术语」或「加入写作库」。加入写作库时可从下拉选择已有写作用途，也可新增类别。
4. 术语和佳句保留来源论文和页码，可从资料库跳回原文；中文由已配置的 LLM 做学术翻译，LibreTranslate 作本地备用翻译。
5. 导出时生成带可见批注的新 PDF 副本，原始文件不修改。
