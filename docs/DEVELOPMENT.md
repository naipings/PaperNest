...
3. **批注（sticky）**：顶栏切到批注模式后单击页面放置便签；拖选文字也可创建并关联引文。便签可拖动、悬停预览、双击编辑；右键复制或删除。
4. 当前页由舞台顶部位置计算；打开后滚动钉在首页，直到用户自己滚动。
...
| 类型 | 绘制方式 |
|------|----------|
| `highlight` | 半透明矩形（按行合并选区） |
| `underline` | 底边线 |
| `text` | 旧版文本批注（兼容渲染为便签） |
| `sticky` | 便签：锚点 + 可选选区；单击放置、拖动、悬停预览、双击编辑 |
| `ink` | 折线路径 |
